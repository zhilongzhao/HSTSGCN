...
class trainer7():
    def __init__(self, in_dim,in_dim_cluster, seq_length, num_nodes, cluster_nodes, dropout, lrate, wdecay, device,
                 supports,supports_cluster,transmit,decay,q,v,m,N,gcn_true,buildA_true,gcn_depth,dilation_exponential,
                 conv_channels,residual_channels,skip_channels,end_channels,layers,propalpha,tanhalpha,subgraph_size,node_dim,semantic_adj,Rsemantic_adj):
        self.model = HSTSGCN(device, num_nodes,cluster_nodes, dropout, supports=supports, supports_cluster=supports_cluster,#support是转换为tensor的邻接矩阵
                           in_dim=in_dim,in_dim_cluster=in_dim_cluster, out_dim=seq_length, transmit=transmit,
                           residual_channels=residual_channels, skip_channels=skip_channels,
                            end_channels=end_channels,q=q,v=v,m=m,N=N,gcn_true=gcn_true,buildA_true = buildA_true,gcn_depth=gcn_depth,dilation_exponential=dilation_exponential,
                           conv_channels=conv_channels,layers=layers,propalpha=propalpha,tanhalpha=tanhalpha,subgraph_size=subgraph_size,node_dim=node_dim,semantic_adj=semantic_adj,Rsemantic_adj=Rsemantic_adj)
        self.model.to(device)
        self.optimizer = optim.Adam(self.model.parameters(), lr=lrate, weight_decay=wdecay)
        self.scheduler = optim.lr_scheduler.ExponentialLR(self.optimizer, decay)
        self.loss = util.masked_mae
