...
#         self.conv2=Conv2d(num_nodes, 1, kernel_size=(1, 1),
#                           stride=(1,1), bias=False)
#         self.w=nn.Parameter(torch.rand(num_nodes,c_in), requires_grad=True)#50,32
#         nn.init.xavier_uniform_(self.w)
#         self.b=nn.Parameter(torch.zeros(tem_size,tem_size), requires_grad=True)#6 6
#
#         self.v=nn.Parameter(torch.rand(tem_size,tem_size), requires_grad=True)#6 6
#         nn.init.xavier_uniform_(self.v)
#         self.bn=BatchNorm1d(tem_size)
#
#     def forward(self,seq):
#         c1 = seq.permute(0,1,3,2)#b,c,n,l->b,c,l,n
#         f1 = self.conv1(c1).squeeze()#b,l,n  x转置乘U1
#
#         c2 = seq.permute(0,2,1,3)#b,c,n,l->b,n,c,l
#         #print(c2.shape)
#         f2 = self.conv2(c2).squeeze()#b,c,n  x转置乘U3
#
#         logits = torch.sigmoid(torch.matmul(torch.matmul(f1,self.w),f2)+self.b)#论文公式9
#         logits = torch.matmul(self.v,logits)
...
class Transmit(nn.Module):
    def __init__(self,c_in,tem_size,transmit,num_nodes,cluster_nodes):
        ...
        self.conv2=Conv2d(tem_size, 1, kernel_size=(1, 1),
                          stride=(1,1), bias=False)
        self.w=nn.Parameter(torch.rand(tem_size,c_in), requires_grad=True)#12/6/3 32
        torch.nn.init.xavier_uniform_(self.w)
        self.b=nn.Parameter(torch.zeros(num_nodes,cluster_nodes), requires_grad=True)#792 50
        self.c_in=c_in
    ...
    def forward(self,seq,seq_cluster):
        ...
        f1 = self.conv1(c1).squeeze(1)#b,n,l
        #f1 = f1.permute(0, 2, 1)
        c2 = seq_cluster.permute(0,3,1,2)#b,c,n,l->b,l,c,n
        f2 = self.conv2(c2).squeeze(1)#b,c,n
        logits=torch.sigmoid(torch.matmul(torch.matmul(f1,self.w),f2)+self.b)
...
#         self.time_conv=Conv2d(c_in, 2*c_out, kernel_size=(1, Kt),padding=(0,0),
#                           stride=(1,1), bias=True,dilation=2)#dilation为2，卷积核元素之间间隔两个像素
#
#         self.multigcn=multi_gcn_time(c_out,2*c_out,Kt,dropout,support_len,order)
#
#         self.num_nodes=num_nodes
...
#         self.bn=BatchNorm2d(c_out)
#
#         self.conv1=Conv2d(c_in, c_out, kernel_size=(1, 1),#c_in, c_out=32，32
#                           stride=(1,1), bias=True)
#
#
#
#
#     def forward(self,x,support):
#         residual = self.conv1(x)#64 32 50 12
#
#         x=self.time_conv(x)#64 64 50 8  这里的卷积核是1*3 且dilation为2就是卷积核各个元素中间空一个 所以卷积完12变为8 可以举个例子看看
#         x1,x2=torch.split(x,[self.c_out,self.c_out],1)#x1 64 32 50 8  x2 64 32 50 8
#         x=torch.tanh(x1)*torch.sigmoid(x2)#这种激活函数的组合可能用于在神经网络中引入非线性，同时保留一定的门控机制。64 32 50 8
#
#
#         x=self.multigcn(x,support)
#         x1,x2=torch.split(x,[self.c_out,self.c_out],1)
...
#         T_coef=T_coef.transpose(-1,-2)
#         x=torch.einsum('bcnl,blq->bcnq',x,T_coef)
#         out=self.bn(x+residual[:, :, :, -x.size(3):])#取出与x维度对应的数据
#         return out
#
